...
from RobustFairCLIP.src.modules1 import *
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            # 1. 基础图文对比损失 (L_contrastive) - 全部样本参与计算以极大化数据利用率
            logits = (image_features @ text_features.T) * torch.exp(torch.tensor(0.07, device=device, dtype=torch.float32))
            ground_truth = torch.arange(len(images), dtype=torch.long, device=device)
            loss_contrastive = (loss_img(logits, ground_truth) + loss_txt(logits.T, ground_truth)) / 2
            
            # 应用噪声/错误样本权重 - 使用Student-t分布
            if (mask[i] and epoch > 0):
                weight = weight_estimation(loss_contrastive.item(), mean_epoch, std_epoch, args.t_dist_dof)
                loss_contrastive = loss_contrastive * weight
            
            # 2. 解剖学正则化损失 (L_anatomy)
            lambda_1 = 1e-3
            loss_anatomy = torch.tensor(0.0, device=device) 
            
            # 3. 公平性损失约束 (L_fairness)
            similarity = (image_features @ text_features.T)
            correlations_with_batch = similarity.diag().float()
            
            # 提取当前batch的属性字典用于掩码屏蔽
            batch_attributes = label_and_attributes[:, 1:].to(device)
            loss_fairness = torch.tensor(0.0, device=device)
            
            # 为每个属性计算公平性损失
            for attr_idx in target_attr_indices:
                attr_name = [k for k, v in attr_to_idx.items() if v == attr_idx][0]
                
                # 核心合规：动态提取合法子群体掩码，屏蔽标记为未明确或未知(-1)的样本
                valid_mask = batch_attributes[:, attr_idx] != -1
                valid_correlations_with_batch = correlations_with_batch[valid_mask]
                
                # 如果当前batch中该属性全是未知样本，则跳过Sinkhorn计算
                if len(valid_correlations_with_batch) == 0:
                    continue
                ...
                for group_loader in group_loaders:
                    images_dist, texts_dist, _ = next(group_loader)
                    ...
                    correlations_with_group /= (correlations_with_group.sum() + 1e-8)
                    
                    # 仅在合法有效样本组成的子分布之间计算距离
                    attr_loss += loss_for_FairCLIP(
                        valid_correlations_with_batch[:, None], 
                        correlations_with_group[:, None]
                    )
                
                loss_fairness += (attr_loss / len(group_loaders))
            
            if len(target_attr_indices) > 0:
                loss_fairness = loss_fairness / len(target_attr_indices)
            
            # 4. 组合最终目标函数方程：L_total = L_contrastive + λ1 * L_anatomy + λ2 * L_fairness
            total_loss = loss_contrastive + lambda_1 * loss_anatomy + args.lambda_fairloss * loss_fairness
